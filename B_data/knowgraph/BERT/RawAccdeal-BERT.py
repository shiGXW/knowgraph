...
def RawAcc_BEONE(excel_datas):
    ...
    delimiters = ["、", "（", "）"]
    # 加载模型
    BBc = BertBase(excel_path + "bert-base-chinese/", "cuda:1")
    # 加载数据，去重
    excel_data = list(set(
        excel_datas[0][0]["materialName"] + excel_datas[1][0]["productName"] + excel_datas[2][0]["material_name"] + excel_datas[3][0]["product_name"]
    ))
    ...
    # 是否继续之前保存
    if os.path.exists(os.path.join(excel_path, 'rawacc_beone_dict.json')):
...
def RawAcc_BEONE_E(excel_datas):
    ...
    delimiters = ["、", "（", "）"]
    # 加载模型
    BBc = BertBase(excel_path + "bert-base-uncased/", "cuda:1")
    # 加载数据，去重
    excel_data = list(set(
        excel_datas[0][0]["materialName"] + excel_datas[1][0]["productName"] + excel_datas[2][0]["material_name"] +
        excel_datas[3][0]["product_name"]
    ))
    catalogue_data = excel_datas[4][0][str(5)]
    ...
    total = len(excel_data)
    # 是否继续之前保存
    if os.path.exists(os.path.join(excel_path, 'rawacc_beone_E_dict.json')):
        with open(os.path.join(excel_path, 'rawacc_beone_E_dict.json'), 'r') as json_file:
            rawacc_beone_dict = json.loads(str(json_file.read()))
        begin = rawacc_beone_dict["saved_current"]
    logging.info(f"Begin index: {begin}")
    for current in range(begin, total):
        ...
        if current % 10 == 0 and current != rawacc_beone_dict["saved_current"] and current != 0:
            ...
            rawacc_beone_dict["saved_current"] = current
            rawacc_beone_dict_json_str = json.dumps(rawacc_beone_dict, indent=4, ensure_ascii=False)
            with open(os.path.join(excel_path, 'rawacc_beone_E_dict.json'), 'w') as json_file:
                json_file.write(rawacc_beone_dict_json_str)
            logging.info(f"saved {current}")
...
if __name__ == '__main__':
    ...
    # 原料、产品实体链接（中文）：rawacc_beone_dict.json
    RawAcc_BEONE(excel_datas)

    # # 翻译
    # catalogue_data_tran(excel_datas)

    # # 原料、产品实体链接（英文）：rawacc_beone_E_dict.json
