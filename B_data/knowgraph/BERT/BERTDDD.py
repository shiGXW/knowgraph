...
class BertBase():
    def __init__(self, model_name, device, catalogue_data, match_data_simple, batch_size=32):
        # 设置输出日志
        ...
        # 标准词向量
        # 按照每批计算词句向量
        catalogue_embeddings = self.batch_sentences(catalogue_data, batch_size)
        match_embeddings = self.batch_sentences(match_data_simple, batch_size)

        # 词句向量放入GPU
        self.catalogue_embeddings = catalogue_embeddings.to(self.device)
        self.match_embeddings = match_embeddings.to(self.device)
    ...
    def train(self, sentences1, sentences2, batch_size=32):
        ...
        if sentences2 == "catalogue":
            similarities, similarity_indexs = self.compare_sentences(sentence_embeddings1, self.catalogue_embeddings)
        else:
            similarities, similarity_indexs = self.compare_sentences(sentence_embeddings1, self.match_embeddings)
